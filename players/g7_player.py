...
class Formation:
    ...
    def get_next_formation_points(self, state, xOffSetCompleted=0):
        '''
...
class Player:
    ...
    def move(self, last_percept, current_percept, info) -> (list, list, int):
        ...
        goalFormation = self.formation.get_next_formation_points(current_percept, xOffSetCompleted)
...
def show_formation_test():
    ...
    formation.update(1)
    points = formation.get_next_formation_points(TestAmoeba(), xOffSetCompleted=0)
    x, y = zip(*points)
    plt.scatter(x, y)
    plt.xticks(range(min(x), max(y)+1))
    plt.savefig("formation.png")
